...
public class TableLoader<TMarshalStruct> : MonoBehaviour
{
    ...
    TableRecordParser<TMarshalStruct> tableRecordParse = new TableRecordParser<TMarshalStruct> ();
    ...
    void ParseTable(string text)
    {
        ...
        while ((line = reader.ReadLine()) != null)          // 파일 끝날 때 까지 계속 레코드 파싱
        {
            if (!fieldRead)
            {
                fieldRead = true;
            ...
            TMarshalStruct data = tableRecordParse.ParseRecordLine(line);
            AddData(data);
        }
